...
for date in trading_dates[100:]:
    ...
    try:
        ...
        for sample_ticker in sample_tickers:
            ...
            ticker_date_universe = full_ticker_prices[(full_ticker_prices["date"] == prior_date) & (full_ticker_prices["ticker"] != sample_ticker)].copy()#.sort_index()
            ...
            for date_ticker in tickers_on_date:
                ...
                ticker_corr_sample = pd.merge(sample_ticker_data, sample_ticker_data_for_comparison, on = "date")
                ...
                ticker_corr_data = pd.DataFrame([{"ticker": date_ticker, "px_corr": px_corr, "ret_corr": ret_corr, "long_wt": (ticker_corr_sample["vol_y"].iloc[-1] / ticker_corr_sample["vol_x"].iloc[-1])}])
            ...
            corr_pair_data = pd.DataFrame([{"date": date, "long_ticker": sample_ticker, "short_ticker": most_correlated["ticker"].iloc[0],
                                            ...
                                            "long_wt": most_correlated["long_wt"].iloc[0]}])
            
            corr_pair_list.append(corr_pair_data)
            
        full_corr_pairs = pd.concat(corr_pair_list).reset_index(drop=True)
        ...
        for pair in full_corr_pairs.index:
            ...
            try:
                ...
                pair_data = full_corr_pairs[full_corr_pairs.index == pair].copy()
                
                long_ticker = pair_data["long_ticker"].iloc[0]
                short_ticker = pair_data["short_ticker"].iloc[0]
                ...
                trade = pd.merge(pair_data, trade, on="date")
